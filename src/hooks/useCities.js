...
import { getCities, getCitiesList, getCityById, getCitiesForVendor, getCountries } from "@/services/citiesServices";
...
export const useCitiesForVendor = (countryCode) => {
    const locale = useLocale();

    console.log("Fetching cities for vendor with countryCode:", countryCode, "and locale:", locale);

    return useQuery({
        // The queryKey includes 'locale' so it auto-refetches if the language changes
        queryKey: ['citiesForVendor', countryCode, locale],
        queryFn: () => getCitiesForVendor(countryCode, locale),
